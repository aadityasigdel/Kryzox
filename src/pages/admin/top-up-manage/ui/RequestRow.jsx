...
const RequestRow = ({ request, onUpdate }) => {
    ...
    const confirmAction = async () => {
        setShowConfirm(false);
        if (!request.topupId) return;

        if (actionType === "approve") {
            setApproveLoading(true);
            console.log("Approved topupId:", request.topupId);
            await updateData(`topup/${request.topupId}/approve`);
            onUpdate?.(request.topupId, "APPROVED");
            setApproveLoading(false);
        } else if (actionType === "reject") {
            setRejectLoading(true);
            console.log("Rejected topupId:", request.topupId);
            await updateData(`topup/${request.topupId}/reject`);
            onUpdate?.(request.topupId, "REJECTED");
        ...
        <>
            <tr className="border-b border-gray-800 hover:bg-gray-800/40 transition">
                ...
                <td className="px-4 py-3">
                    <span
                        className={`px-2 py-1 rounded ${statusColors[request.status] || "bg-gray-700 text-gray-300"
                            }`}
                    >
                    ...
                    {request.status === "PENDING" && (
                        <>
                            <button
                                ...
                                className={`px-4 py-2 rounded font-semibold text-white transition-colors duration-200 ${approveLoading
                            ...
                            <button
                                ...
                                disabled={approveLoading || rejectLoading}
                                className={`px-4 py-2 rounded font-semibold text-white transition-colors duration-200 ${rejectLoading
                                        ? "opacity-50 cursor-not-allowed"
                                        : "bg-gradient-to-r from-[#B05BDB] to-[#80FFDB] hover:from-[#C06BDD] hover:to-[#1ac595]"
                                    }`}
                            >
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black/60 z-50">
                    <div className="bg-gradient-to-b from-[#202020] to-[#111] rounded-lg p-5 w-80 max-w-[90%] text-center shadow-lg border border-[#B05BDB]">
                        ...
                        <p className="text-gray-300 mb-4 text-sm">
                            Are you sure you want to {actionType === "approve" ? "approve" : "reject"} this topup? <br />
