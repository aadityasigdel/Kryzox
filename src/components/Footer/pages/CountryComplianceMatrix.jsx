...
const CountryComplianceMatrix = () => {
  const complianceData = [
    ...
    { country: "Nepal", regulations: "Electronic Transactions Act, Consumer Laws" },
    ...
    { country: "Latin America", regulations: "Localized digital laws per country" },
    { country: "Others", regulations: "ISO/IEC 27001 standards applied where possible" },
  ];
      ...
      <div className="max-w-5xl mx-auto">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-700">
                <th className="px-4 py-2 text-gray-300 text-lg font-semibold">Country/Region</th>
                <th className="px-4 py-2 text-gray-300 text-lg font-semibold">Applicable Regulations</th>
              </tr>
            </thead>
            <tbody>
              {complianceData.map((item, index) => (
                <tr key={index} className={index % 2 === 0 ? "bg-[#241B3A]" : "bg-[#1B1233]"}>
                  <td className="px-4 py-2 text-gray-300">{item.country}</td>
                  <td className="px-4 py-2 text-gray-300">{item.regulations}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="mt-8">
          <p className="text-gray-300 leading-relaxed">
            Users are advised to check with their local authority or seek legal counsel if unsure about compliance.
          </p>
          <p className="text-gray-300 leading-relaxed mt-2">
            For legal or regulatory questions, please contact: <span className="text-purple-300">kryzox@gmail.com</span>
          </p>
        </div>
      </div>
    </div>
