...
export default function HeroSection() {
  return (
    <div className="relative min-h-screen w-full bg-[#0b001a] text-white overflow-hidden font-sans pb-5">
      {/* ✅ Vertical Lines */}
      {[...Array(7)].map((_, i) => (
        <div
          key={i}
      ...
      {[...Array(10)].map((_, i) => (
        <div
          key={`dot-${i}`}
          className="absolute w-2 h-2 bg-white rounded-full blur-sm opacity-40 animate-bounce z-0"
          style={{
            top: `${Math.random() * 95}%`,
            left: `${Math.random() * 95}%`,
